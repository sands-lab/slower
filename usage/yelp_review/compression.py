...
def compress_embeddings(embeddings, lens):
    """Here the logic is the following: if a token (and therefore its embedding) is not attended, its value is irrelevant
    """
    compressed_data = []
...
def get_extended_attention_mask(batch_size, max_context_size, lens):
    ...
    attention_mask = 1.0 - (torch.arange(max_context_size).expand(batch_size, max_context_size) < lens.unsqueeze(1)).int()
